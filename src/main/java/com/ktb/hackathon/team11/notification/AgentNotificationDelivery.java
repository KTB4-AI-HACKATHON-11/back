...
@Entity
@Table(
    name = "agent_notification_deliveries",
    uniqueConstraints =
        @UniqueConstraint(
            name = "uk_agent_notification_delivery_target",
            columnNames = {"outbox_id", "subscription_id"}),
    indexes =
        @Index(
            name = "idx_agent_notification_delivery_pending",
...
public class AgentNotificationDelivery extends BaseEntity {
  ...
  private AgentNotificationOutbox outbox;
  ...
  public AgentNotificationDelivery(
      AgentNotificationOutbox outbox,
