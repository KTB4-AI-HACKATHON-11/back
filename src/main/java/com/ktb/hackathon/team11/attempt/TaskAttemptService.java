...
public class TaskAttemptService {
  ...
  @Value("${storage.presigned-url-minutes:5}")
  private long urlMinutes;
  ...
  private void evaluate(
    ...
    try {
      ...
      try {
        ...
        result =
            check(
                ...
                size,
                sha,
                storage.createReadUrl(objectKey, Duration.ofMinutes(urlMinutes)));
      }
      ...
      if (exception.getErrorCode() != ErrorCode.AI_UNAVAILABLE
          && exception.getErrorCode() != ErrorCode.PHOTO_UNAVAILABLE) throw exception;
      attempt.delayed();
      assignment.delayed();
    }
  }
  ...
  private PhotoCheckResult check(
      TaskAssignment assignment, String mime, long size, String sha, String url) {
