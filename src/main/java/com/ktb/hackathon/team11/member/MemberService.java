...
public class MemberService {
    ...
    public Member create(String nickname, MemberRole role) {
        ...
        if (repository.existsByNickname(normalized)) throw new BusinessException(ErrorCode.DUPLICATE_NICKNAME);
        return repository.save(new Member(normalized, role));
    }

    public Member login(String nickname) {
        return repository.findByNickname(nickname.strip()).orElseThrow(() -> new BusinessException(ErrorCode.MEMBER_NOT_FOUND));
    }

    public Member requireMember(long id) {
        return repository.findById(id).orElseThrow(() -> new BusinessException(ErrorCode.MEMBER_NOT_FOUND));
    }

    public Member requireRole(long id, MemberRole role) {
        Member member = requireMember(id);
        if (member.getRole() != role) throw new BusinessException(role == MemberRole.MANAGER ? ErrorCode.MANAGER_REQUIRED : ErrorCode.WORKER_REQUIRED);
        return member;
    }
}
