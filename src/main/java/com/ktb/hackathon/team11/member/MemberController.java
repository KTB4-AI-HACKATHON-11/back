...
public class MemberController {
    ...
    @Operation(
            ...
            responses = {
                    ...
                    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400", description = "닉네임 또는 역할 형식 오류"),
    ...
    @Operation(
            ...
            responses = {
                    ...
                    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "존재하지 않는 닉네임")
            }
    )
    @PostMapping("/login")
    ...
    public record CreateMemberRequest(
            @Schema(description = "중복되지 않는 닉네임", example = "야간알바")
            @NotBlank @Size(max = 30) String nickname,
            @Schema(description = "서비스 역할", example = "WORKER", allowableValues = {"MANAGER", "WORKER"})
            @NotNull MemberRole role
    ) {}

    @Schema(description = "데모 로그인 요청")
    public record LoginRequest(
            @Schema(description = "가입할 때 사용한 닉네임", example = "야간알바")
